...
// All book-specific identity data comes from book.env via book-info.typ.
...
#import "book-info.typ": book_info
...
#let isbn_block = if book_info.hardcover_isbn == "" and book_info.paperback_isbn == "" {
  none
} else {
  if book_info.hardcover_isbn != "" {
    [ISBN: #text(book_info.hardcover_isbn) (hardcover)]
  }
  if book_info.hardcover_isbn != "" and book_info.paperback_isbn != "" {
    linebreak()
  }
  if book_info.paperback_isbn != "" {
    [ISBN: #text(book_info.paperback_isbn) (paperback)]
  }
}
...
#book.copyright_page(
  ...
  year: book_info.copyright_year,
  publication: publication,
  source_note: if book_info.source_title == "" {
    none
  } else {
    [This booklet is excerpted from #emph[#text(book_info.source_title)] by #text(book_info.author).]
  },
  scripture_notice: if book_info.scripture_notice == "" { none } else { book_info.scripture_notice },
  isbn_block: isbn_block,
)
